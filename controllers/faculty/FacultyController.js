...
const FacultyController = {
  ...
  signUp: async (req, res) => {
    try {
      ...
      new FacultyModel().addData(
        DB_Define.FACULTY_TABLE,
        faculty,
        (err, results) => {
          if (err) {
            ...
            res
              .status(200)
              .json(
                new Response(false, "faculty created successfully", faculty)
              );
          }
        }
      ); //end db
  ...
  login: async (req, res) => {
    try {
      ...
      new FacultyModel().getFacultyByEmail(
        DB_Define.FACULTY_TABLE,
        email,
        async (err, results) => {
          try {
            if (err) {
              ...
              res
                .status(200)
                .json(
                  new Response(false, "faculty logged in successfully", faculty)
                );
            }
          } catch (e) {
            let response = new Response(true, e.message, e);

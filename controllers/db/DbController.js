...
const DbController = {
  ...
  createTicketChatTable: (req, res) => {
    new DbModel().create_ticket_chat_table((err, results) => {
      if (err) {
        ...
        let response = new Response(
          false,
          " ticket_chat table Created Successfully",
          results
        );
        res.status(200).send(response);
      }
  ...
  createNoticeTable: (req, res) => {
    new DbModel().create_notice_table((err, results) => {
      if (err) {
        ...
        let response = new Response(
          false,
          " Notice table Created Successfully",
          results
        );
        res.status(200).send(response);
      }
  ...
  createSubSkillTable: (req, res) => {
    new DbModel().create_sub_skill_table((err, results) => {
      if (err) {
        ...
        let response = new Response(
          false,
          "sub skill table Created Successfully",
          results
        );
        res.status(200).send(response);
      }
  ...
  createStudentSkillListTable: (req, res) => {
    new DbModel().createStudentSkillListTable((err, results) => {
      if (err) {
        ...
        let response = new Response(
          false,
          "StudentSkillList table Created Successfully",
          results
        );
        res.status(200).send(response);
      }
  ...
  //create faculty task
  createFacultyTaskTable: (req, res) => {
    new DbModel().create_faculty_task_table((err, results) => {
      if (err) {
        let response = new Response(true, err.message, err);
        res.send(response);
      } else {
        let response = new Response(
          false,
          "FacultyTaskTable Created Successfully",
          results
        );
        res.status(200).send(response);
      }
